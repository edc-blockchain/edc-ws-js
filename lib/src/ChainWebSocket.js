...
if(typeof WebSocket === "undefined" && !process.env.browser) {
	...
	WebSocketClient = require("robust-websocket")
}
...
class ChainWebSocket {
	...
	constructor(ws_server, statusCb, autoReconnect) {

		this.statusCb = statusCb;

		this.ws = new WebSocketClient(ws_server, null, {
			timeout: 5000,
			shouldReconnect: function(event, ws) {
				return autoReconnect ? 30000 : false
			}
		});
	...
	call(params) {
		...
		if(method === "set_subscribe_callback" || method === "subscribe_to_market" ||
			method === "broadcast_transaction_with_callback_new" || method === "set_pending_transaction_callback"
		) {

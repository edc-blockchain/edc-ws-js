...
class ApisSingleton {
	 ...
	 */
	reset(cs = "ws://localhost:8090", connect, user, password, reconnectAttempts = null) {
		if(this.inst) {
			this.inst.close();
			this.inst = null;
		}
		...
		if(this.inst && connect) {
			this.inst.connect(cs, user, password, reconnectAttempts);
		}
		
		return this.inst;
	}
	instance(cs = "ws://localhost:8090", connect, user, password, reconnectAttempts = null) {
		if(!this.inst) {
			this.inst = new ApisInstance();
			this.inst.setRpcConnectionStatusCallback(this.statusCb);
		}
